...
The execution is divided into the following stages:
  ...
  - **PICK_SHELF:** The platform is raised to lift the shelf, then the shelf is removed from the map by marking its pixels as free using the **remove_shelf** function, since while it is raised, the shelf will no longer be an obstacle. Finally, the new location corresponding to the current shelf is selected and the state is returned to **PLANNING**.
